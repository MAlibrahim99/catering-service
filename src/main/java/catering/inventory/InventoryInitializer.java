...
class InventoryInitializer implements DataInitializer {
	...
	public void initialize() {
		...
		inventory.save(new UniqueInventoryItem(catalog.findByName("Salatplatte").stream().findFirst().get(),
				Quantity.of(100)));
		inventory.save(new UniqueInventoryItem(catalog.findByName("Sushi").stream().findFirst().get(),
				Quantity.of(100)));
		inventory.save(new UniqueInventoryItem(catalog.findByName("Pizza").stream().findFirst().get(),
				Quantity.of(100)));
		...
		inventory.save(new UniqueInventoryItem(catalog.findByName("Alkoholfreie Getränke").stream().findFirst().get(),
				Quantity.of(100)));
		inventory.save(new UniqueInventoryItem(catalog.findByName("Frühstück").stream().findFirst().get(),
				Quantity.of(100)));
